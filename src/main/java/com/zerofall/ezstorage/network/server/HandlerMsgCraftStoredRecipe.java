...
public class HandlerMsgCraftStoredRecipe implements IMessageHandler<MsgCraftStoredRecipe, IMessage> {

    @Override
    public IMessage onMessage(MsgCraftStoredRecipe message, MessageContext ctx) {
        ...
        if (message.index < 0 || message.index >= con.inventory.recipes.size() || message.count <= 0) {
            return null;
        }
